...
impl Monkey {
    ...
    fn get_items(mut items_str: String) -> VecDeque<u64> {
        items_str.drain(..STARTING_ITEMS.len());
        if items_str.contains(",") {
            items_str.split(", ").into_iter().map(|i| i.trim().parse::<u64>().unwrap()).collect()
        } else {
            let mut items = VecDeque::new();
            items.push_back(items_str.trim().parse::<u64>().unwrap());
            items
        }
    }

    fn get_operation(mut operation_str: String) -> Box<dyn Fn(u64) -> u64> {
        ...
        match operation_elements[1] {
            ...
            _ => {
                let y = operation_elements[1].to_string().parse::<u64>().unwrap();
                if operation_elements[0] == "+" {
                    Box::new(move |x| x + y)
                } else {
                    Box::new(move |x| x * y)
    ...
    pub fn operates(&mut self, modulo: u32) {
        self.inspection_counter += 1;
        let item = self.items[0];
        self.items[0] = ((self.operation)(item)) % modulo as u64;
    }
...
impl Test {
    ...
    fn test(&self, value: u64) -> u32 {
        if value % self.divider as u64 == 0 {
            self.if_successful
        } else {
            self.if_failed
        }
