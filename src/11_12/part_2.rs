...
fn get_level_monkey_business(mut monkeys: Vec<Monkey>) -> u64 {
    let number_monkey = monkeys.len();
    let modulo = monkeys.iter().map(|m| m.test.divider).product();
    ...
    println!("{largest_number_inspection_1}, {largest_number_inspection_2}");
    largest_number_inspection_1 as u64 * largest_number_inspection_2 as u64
}
