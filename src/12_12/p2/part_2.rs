...
fn main() {
    ...
    let lowest_step = get_lowest_step(tree, possible_starts);
    println!("Plus petit nombre de pas : {lowest_step}");
}
...
fn get_way(map: Vec<Vec<u32>>) -> Arc<Mutex<Tree>> {
    let initial_position = get_position(&map, 123);

    let tree = Arc::new(Mutex::new(Tree::new(initial_position.clone(), 123)));
    let lock_tree = tree.lock().unwrap();
    let actual_node = lock_tree.get_existing_node_by_position(initial_position.clone()).unwrap();
    drop(lock_tree);

    find_path(&map, actual_node, Arc::clone(&tree),  vec![initial_position], 0);
    tree
}
...
fn find_path(map: &Vec<Vec<u32>>, actual_node: Arc<Mutex<Node>>, tree: Arc<Mutex<Tree>>, positions_visited: Vec<(i32, i32)>, mut step: u32) {
    step += 1;
    ...
    let possible_ways = get_possible_ways(map, current_position, actual_value, &positions_visited);
    ...
    for possible_way in &possible_ways {
        match lock_tree.get_existing_node_by_position(possible_way.coordinate) {
            Some(node) => {
                ...
                if step < lock_node.position {
                    lock_node.position = step;
                    drop(lock_node);
                    nodes_to_continue.push(node);
                }
            },
            None => {
                let node = Arc::new(Mutex::new(Node::new(possible_way.coordinate, possible_way.value, step)));
    ...
    for node in nodes_to_continue {
        ...
        let handler = thread::spawn(move || {
            find_path(&map, actual_node, tree, positions_visited, step)
        });
...
fn get_possible_ways(map: &Vec<Vec<u32>>, current_position: (i32, i32), actual_value: u32, positions_visited: &Vec<(i32, i32)>) -> Vec<PossibleWay> {
    ...
    for i in 0..4 {
        ...
        if value_of_the_next_position < actual_value {
            if actual_value - value_of_the_next_position > 1 {
                continue;
            }
        }
...
fn get_lowest_step(tree: Arc<Mutex<Tree>>, possible_starts: Vec<(i32, i32)>) -> u32 {
    let mut lowest_step = u32::MAX;
    let lock_tree = tree.lock().unwrap();

    for possible_start in possible_starts {
        match lock_tree.get_existing_node_by_position(possible_start) {
            Some(node) => {
                let lock_node = node.lock().unwrap();
                let value = lock_node.point.value;
                let step = lock_node.position;
                println!("Node :\n- value : {value},\n- step : {step}");
                if step < lowest_step {
                    println!("Est plus petit");
                    println!("Localisation : {:?}", possible_start);
                    println!("Node :\n- value : {value},\n- step : {step}");
                    lowest_step = step;
                }
            },
