...
fn main() {
    ...
    let floor = get_floor_coordinate(&rock_structure_path);
    println!("floor : {floor}");

    let sand_unit_count_before_fall = get_sand_unit_count_until_stopping_flow(rock_structure_path, ORIGIN, floor);
...
fn get_floor_coordinate(rock_structure_path: &HashSet<Line>) -> i32 {
    let mut horizontal_lines = rock_structure_path.iter().filter(|l| l.direction == Direction::Horizontal).collect::<Vec<&Line>>();
    ...
    let mut vertical_lines = rock_structure_path.iter().filter(|l| l.direction == Direction::Vertical).collect::<Vec<&Line>>();
    vertical_lines.sort_by(|a, b| a.vertex_b.cmp(&b.vertex_b));
    vertical_lines.reverse();
    let vertical_line = vertical_lines.get(0).unwrap();

    vertical_line.get_highest_y_coordinate(horizontal_line) + 2
}
...
fn get_sand_unit_count_until_stopping_flow(rock_structure_path: HashSet<Line>, origin: (i32, i32), floor: i32) -> u32 {
...
fn fall_continuing(
    ...
    if fall_from_origin {
        falling_sand_grain = get_next_drop_point(rock_structure_path, sand_grains, &origin, floor);
        if falling_sand_grain.1 < origin.1 {
            return false;
        }
    ...
    loop {
        ...
        if can_move(rock_structure_path, sand_grains, &next_position, floor) {
            ...
            // can move go down ?
            if can_move(rock_structure_path, sand_grains, &next_position, floor) {
                let next_position = get_next_drop_point(rock_structure_path, sand_grains, &next_position, floor);
                falling_sand_grain = next_position;
...
fn can_move(rock_structure_path: &HashSet<Line>, sand_grains: &Vec<(i32, i32)>, next_position: &(i32, i32), floor: i32) -> bool {
    if sand_grains.contains(&next_position) || next_position.1 >= floor {
        return false;
    }
...
fn get_next_drop_point(rock_structure_path: &HashSet<Line>, sand_grains: &Vec<(i32, i32)>, next_position: &(i32, i32), floor: i32) -> (i32, i32) {
    ...
    if highest_landing.is_some() || !sand_grains_on_the_fall.is_empty() {
        match sand_grains_on_the_fall.get(0) {
            None => {
                return (next_position.0, highest_landing.unwrap());
            },
            Some(v) => {
                if highest_landing.is_some() && highest_landing.as_ref().unwrap() < &v.1 {
                    return (next_position.0, highest_landing.unwrap());
                } else {
                    return (next_position.0, v.1 - 1);
                }
            }            
        }
    }

    (next_position.0, floor - 1)
}
